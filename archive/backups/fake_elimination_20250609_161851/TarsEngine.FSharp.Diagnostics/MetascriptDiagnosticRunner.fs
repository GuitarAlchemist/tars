...
type MetascriptDiagnosticRunner(
    ...
    member this.RunWithDiagnostics(metascriptPath: string, ?projectPath: string, ?enableSemanticAnalysis: bool) =
        task {
            ...
            logger.LogInformation("ðŸ” Starting metascript execution with full diagnostics")
            logger.LogInformation("ðŸ“„ Metascript: {MetascriptPath}", metascriptPath)
            
            if not (File.Exists(metascriptPath)) then
                logger.LogError("âŒ Metascript file not found: {MetascriptPath}", metascriptPath)
            ...
            try
                ...
                logger.LogInformation("ðŸ” Started diagnostic trace: {TraceId}", traceId)
                ...
                logger.LogInformation("âœ… Metascript execution completed with diagnostics")
                logger.LogInformation("ðŸ“Š Trace file: {TraceFile}", traceFile)
                
                if enableSemantic then
                    logger.LogInformation("ðŸ§  Generating semantic analysis report...")
                    let! semanticReportFile = semanticAnalyzer.GenerateComprehensiveDiagnosticReport(trace)
                    logger.LogInformation("ðŸ“‹ Semantic report: {SemanticReportFile}", semanticReportFile)
            ...
            with
            | ex ->
                logger.LogError(ex, "âŒ Error during metascript execution with diagnostics")
    ...
    member private this.ExecuteMetascriptWithDiagnostics(metascriptPath: string) =
        task {
            logger.LogInformation("ðŸš€ Executing metascript with diagnostic recording...")
            ...
            logger.LogInformation("âœ… Metascript execution simulation completed")
        }
    
    /// Run diagnostic analysis on existing trace file
    member this.AnalyzeExistingTrace(traceFilePath: string) =
        task {
            logger.LogInformation("ðŸ” Analyzing existing trace file: {TraceFilePath}", traceFilePath)
            
            if not (File.Exists(traceFilePath)) then
                logger.LogError("âŒ Trace file not found: {TraceFilePath}", traceFilePath)
            ...
            try
                ...
                logger.LogInformation("ðŸ§  Generating semantic analysis for existing trace...")
                let! semanticReportFile = semanticAnalyzer.GenerateComprehensiveDiagnosticReport(trace)
                
                logger.LogInformation("ðŸ“‹ Semantic analysis complete: {SemanticReportFile}", semanticReportFile)
            ...
            with
            | ex ->
                logger.LogError(ex, "âŒ Error analyzing existing trace file")
                return Error ex.Message
        }
    ...
    member this.ListTraceFiles() =
        ...
        if not (Directory.Exists(tracesDir)) then
            logger.LogInformation("ðŸ“ No traces directory found")
            []
        else
            ...
            logger.LogInformation("ðŸ“Š Found {TraceCount} trace files and {ReportCount} reports", 
