...
- [x] **2.5.2 Units of Measure** (Implemented in `src/Tars.Core/Units.fs`)
  - [x] Define `[<Measure>] type token`
  - [x] Define `[<Measure>] type ms`
  - [x] Update `BudgetGovernor` to use these (see `Budget.fs`)
