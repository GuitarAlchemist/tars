...
- **Memory migration note**: include a migration stub that defines the version numbering scheme, a required backfill script, and the decision point for promoting to a vector DB.
  - **V1 reuse strategy**: Port v1's `InMemoryVectorStore` (~70% code reuse) by keeping the `IVectorStore` interface and file-based persistence while removing multi-space embeddings (hyperbolic, FFT, Pauli, etc.).
  - **Simplified embedding**: Use raw float vectors only with metadata, checksums, and version tracking.
  - **Migration trigger**: Promote to external vector DB (Qdrant/Pinecone) when file count exceeds 10K documents or search latency exceeds 500ms.
  - **Interface stability**: `IVectorStore` abstraction allows backend swap without changing consumers.
...
- **Story:** Port and simplify v1's `InMemoryVectorStore` - keep the `IVectorStore` interface, file-based JSON persistence, and CRUD operations; remove multi-space embeddings (hyperbolic, FFT, Pauli matrices, etc.) and use simple raw vectors only; add checksum/version metadata per v2 requirements.
- **Story:** Define migration path interface from file-based store to external vector DB (Qdrant/Pinecone) for future phases.
