...
module WotGoldenRunTests =
    ...
    let ``executePlanV0 - golden run resolves vars, invokes tool, verifies, traces`` () =
        ...
        let steps: Step list =
            [ // Work: ToolCall echoes target_msg into res1
              { Id = "tool1"
                Inputs = []
                Outputs = [ "res1" ]
                Action = StepAction.Work(WorkOperation.ToolCall("mock_echo", Map.ofList [ "msg", box "${target_msg}" ]))
                Agent = None
                Metadata = Map.empty }
              ...
              { Id = "verify1"
                ...
                Action =
                  StepAction.Work(WorkOperation.Verify [ WotCheck.NonEmpty "res1"; WotCheck.Contains("res1", "Hello") ])
                Agent = None
                Metadata = Map.empty } ]
        ...
        // Assert
        match result with
        | Error(err, trace) -> failwith $"Expected Ok, got Error: {err}\nTrace count: {trace.Length}"
        | Ok(ctx, lastVerify, trace) ->
            ...
            | Some vr -> Assert.True(vr.Passed, $"Verification failed: %A{vr.Errors}")
    ...
    let ``executePlanV0 - reason step invokes IReasoner when mode is Llm`` () =
        ...
        let reasoner =
            { new IReasoner with
                member _.Reason(stepId, ctx, goal, _, _) =
                    async {
                        reasonerCalled <- true
    ...
    let ``executePlanV0 - is deterministic (golden snapshot equality)`` () =
        ...
        let steps =
            [ { Id = "step1"
                Inputs = []
                Outputs = [ "o1" ]
                Action = StepAction.Reason(ReasonOperation.Explain "Something")
                Agent = None
                Metadata = Map.empty } ]
        ...
        let reasoner =
            { new IReasoner with
                member _.Reason(_, _, _, _, _) =
                    async { return Ok { Content = "Output"; Usage = None } } }
