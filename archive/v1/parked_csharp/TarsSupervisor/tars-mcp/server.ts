...
const PROJECT =
  process.env.TARS_SUPERVISOR_DIR || process.cwd(); // folder with TarsSupervisor.Cli
...
function latestVersionDir(base: string) {
...
server.tool(
  ...
  async () => ({ content: [{ type: "text", text: "pong from tars-mcp" }] })
);

server.tool(
  "tars.plan",
  "Generate PLAN.md and next_steps.trsx via the supervisor",
  { type: "object", properties: {}, additionalProperties: false },
  async () => {
    const { code, out, err } = await run([...DOTNET, "plan"]);
    if (code !== 0) throw new Error(err || out || `plan exited ${code}`);
...
server.tool(
  "tars.iterate",
  "Run full iteration (plan -> validate -> metrics)",
  { type: "object", properties: {}, additionalProperties: false },
  async () => {
    const { code, out, err } = await run([...DOTNET, "iterate"]);
    if (code !== 0) throw new Error(err || out || `iterate exited ${code}`);
...
server.tool(
  "tars.report",
  "Summarize runs from metrics.jsonl",
  { type: "object", properties: {}, additionalProperties: false },
  async () => {
    const { code, out, err } = await run([...DOTNET, "report"]);
    if (code !== 0) throw new Error(err || out || `report exited ${code}`);
    return { content: [{ type: "text", text: out }] };
  }
);
...
server.tool(
  "tars.rollback",
  "Point CURRENT to last SUCCESS version",
  { type: "object", properties: {}, additionalProperties: false },
  async () => {
    const { code, out, err } = await run([...DOTNET, "rollback"]);
    if (code !== 0) throw new Error(err || out || `rollback exited ${code}`);
    return { content: [{ type: "text", text: out }] };
  }
);
  ...
  async () => {
    ...
    const dirs = existsSync(root)
      ? readdirSync(root, { withFileTypes: true })
          ...
          .reverse()
      : [];
    if (!dirs[0]) throw new Error("No versions yet");
...
server.resource(
  "Latest next_steps.trsx",
  "tars://latest/next_steps",
  async () => {
    const root = join(PROJECT, "output", "versions");
    const dirs = existsSync(root)
      ? readdirSync(root, { withFileTypes: true })
