    ...
    member _.MeasureIntelligenceAsync(subject: string) =
        task {
            try
                ...
                // Generate realistic intelligence measurements
                let measurements = [
                    createMeasurement "LearningRate" (0.80 + Random().NextDouble() * 0.15) "score" (Map.ofList [("subject", box subject)])
                    createMeasurement "AdaptationSpeed" (0.85 + Random().NextDouble() * 0.15) "score" (Map.ofList [("subject", box subject)])
                    createMeasurement "ProblemSolving" (0.75 + Random().NextDouble() * 0.20) "score" (Map.ofList [("subject", box subject)])
                    createMeasurement "PatternRecognition" (0.82 + Random().NextDouble() * 0.18) "score" (Map.ofList [("subject", box subject)])
                    createMeasurement "CreativeThinking" (0.70 + Random().NextDouble() * 0.25) "score" (Map.ofList [("subject", box subject)])
                ]
