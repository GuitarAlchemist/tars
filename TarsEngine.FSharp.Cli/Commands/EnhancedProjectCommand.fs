...
type EnhancedProjectCommand(logger: ILogger<EnhancedProjectCommand>) =
    interface ICommand with
        ...
        member _.Usage = "tars create-project \"<prompt>\" [options]"
        member _.Examples = [
            ...
            task {
                try
                    ...
                    | prompt :: _ ->
                        ...
                        do! this.GenerateProjectFiles(projectPath, projectName, fullPrompt, projectType)
                        
                        // Generate comprehensive documentation
                        do! this.GenerateDocumentation(projectPath, projectName, fullPrompt, projectType)
                        
                        // Generate run.cmd script
                        do! this.GenerateRunScript(projectPath, projectName, projectType)
                        
                        // Generate package.json or project file
                        do! this.GenerateProjectConfig(projectPath, projectName, fullPrompt, projectType)
    ...
    member private this.GenerateProjectFiles(projectPath: string, projectName: string, prompt: string, projectType: string) =
        task {
...
namespace %s
{
    ...
    member private this.GenerateDocumentation(projectPath: string, projectName: string, prompt: string, projectType: string) =
        task {
...
""" projectName projectType 
    ...
    member private this.GenerateRunScript(projectPath: string, projectName: string, projectType: string) =
        task {
...
call npm start"""
    ...
    member private this.GenerateProjectConfig(projectPath: string, projectName: string, prompt: string, projectType: string) =
        task {
            match projectType with
            | "web" | "api" ->
