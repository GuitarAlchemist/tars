                ...
                | Some task ->
                    ...
                    | Some fnName ->
                        ...
                        | Some fn ->
                            ...
                            Interpreter.executeBlock fn.Block fnEnv
                        | None -> Error $"Function '{fnName}' not found in task '{taskName}' for agent '{agentName}'"
                    | None ->
                        ...
                        | Some (_, fn) ->
                            ...
                            // Execute the function block
                            Interpreter.executeBlock fn.Block fnEnv
                        | None -> Error $"No functions found in task '{taskName}' for agent '{agentName}'"
                | None -> Error $"Task '{taskName}' not found for agent '{agentName}'"
            | None -> Error $"Agent '{agentName}' not found"
                ...
                let functions = 
                    ...
                    |> List.map (fun fnBlock ->
                        ...
                        let parameters = 
                            ...
                            | Some (ListValue params) -> 
                                params |> List.choose (function 
                                    | StringValue param -> Some param 
                                    | _ -> None)
                            | _ -> []
