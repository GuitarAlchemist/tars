    ...
    member _.``Golden Run: Demo Ping`` () =
        ...
        let repoRoot = 
            ...
            let rec findRoot (dir: DirectoryInfo) =
                if isNull dir then failwith "Could not find repo root"
                if dir.GetFiles("Tars.sln").Length > 0 then dir
                else findRoot dir.Parent
            findRoot d
        
        output.WriteLine($"Repo root: {repoRoot.FullName}")

        let cliProject = Path.Combine(repoRoot.FullName, "src", "Tars.Interface.Cli", "Tars.Interface.Cli.fsproj")
