...
module Parser =
    ...
    let parseJson (json: string) : ParseResult<Workflow> =
        ...
        | :? JsonException as ex ->
            ...
            let line = if ex.LineNumber.HasValue then int ex.LineNumber.Value else 0
            let col = if ex.BytePositionInLine.HasValue then int ex.BytePositionInLine.Value else 0
            ParseError(line, col, ex.Message)
        | ex ->
            ParseError(0, 0, ex.Message)
    ...
    type WorkflowBuilder(name: string) =
        ...
        member this.Input(n: string, typ: string, desc: string) =
            inputs <- inputs @ [{ Name = n; Type = typ; Description = desc }]
            this
