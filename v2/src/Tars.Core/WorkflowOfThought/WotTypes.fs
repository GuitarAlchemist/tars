...
module NodeId =
    let create () = NodeId (Guid.NewGuid())
...
module NodeContent =
    let empty = { Text = ""; Structured = Map.empty; Confidence = 0.0; Sources = [] }
    let ofText text = { Text = text; Structured = Map.empty; Confidence = 0.5; Sources = [] }
...
type WorkNode = {
    ...
    Policy: PolicyGate
    Evidence: NodeEvidence option
    SideEffects: string list       // What this node changed
}
...
module PolicyEnforcement =
    ...
    let redact (content: string) (sensitive: SensitiveContent list) : string =
        sensitive
        |> List.fold (fun text s ->
            match s with
            | PII "SSN" -> 
                System.Text.RegularExpressions.Regex.Replace(text, @"\b\d{3}-\d{2}-\d{4}\b", "[REDACTED-SSN]")
            | PII "Email" ->
                System.Text.RegularExpressions.Regex.Replace(text, @"\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}\b", "[REDACTED-EMAIL]")
            | PII "Credit Card" ->
...
module WorkflowExecution =
    ...
    let init (graph: WorkflowGraph) : WorkflowState =
        let nodeStates = 
            graph.Nodes 
            |> Map.map (fun _ _ -> Pending)
            |> Map.add graph.EntryPoint Ready
        {
            Graph = graph
...
type IWotEngine =
    ...
    abstract member ExecuteNodeAsync: node: WotNode * state: WorkflowState -> Task<Result<NodeContent * NodeEvidence, string>>
