...
let private checkEnvConfig () =
    ...
    checks |> List.map (fun (key, result) ->
        match result with
        | Ok value ->
            let masked = if value.Length > 4 then value.Substring(0, 4) + "****" else "****"
            { Name = key; Status = "Set"; Details = masked; IsOk = true }
        | Error _ ->
            { Name = key; Status = "Not Set"; Details = "(using defaults)"; IsOk = false }
    )
...
let private getSystemInfo () =
    ...
    [
        { Name = "OS"; Status = "Info"; Details = $"{osDesc} ({osArch})"; IsOk = true }
        { Name = ".NET Runtime"; Status = "Info"; Details = dotnetVersion; IsOk = true }
        { Name = "Process Arch"; Status = "Info"; Details = procArch; IsOk = true }
        { Name = "CPU Cores"; Status = "Info"; Details = $"{procCount} logical processors"; IsOk = true }
    ]
...
let private getMemoryInfo () =
    ...
    let status = if workingSetMB < 500L then "OK" else if workingSetMB < 1000L then "High" else "Critical"
    let isOk = workingSetMB < 1000L

    [
        { Name = "Working Set"; Status = status; Details = $"{workingSetMB} MB"; IsOk = isOk }
        { Name = "Private Memory"; Status = "Info"; Details = $"{privateMemMB} MB"; IsOk = true }
        { Name = "GC Heap"; Status = "Info"; Details = $"{gcTotalMB} MB"; IsOk = true }
    ]
...
let private getGpuInfo () =
    ...
    try
        let psi = System.Diagnostics.ProcessStartInfo("nvidia-smi", "--query-gpu=name,memory.total,driver_version --format=csv,noheader,nounits")
        ...
        if proc.ExitCode = 0 && not (String.IsNullOrWhiteSpace(output)) then
            ...
            for line in lines do
                ...
                if parts.Length >= 3 then
                    ...
                    let memMB = parts.[1]
                    let driver = parts.[2]
                    results.Add({ Name = "NVIDIA GPU"; Status = "Detected"; Details = $"{gpuName}, {memMB} MB VRAM, Driver {driver}"; IsOk = true })

            // Check CUDA version
            let psiCuda = System.Diagnostics.ProcessStartInfo("nvidia-smi", "--query-gpu=compute_cap --format=csv,noheader")
            ...
            if not (String.IsNullOrWhiteSpace(cudaOutput)) then
                results.Add({ Name = "CUDA Compute"; Status = "Available"; Details = $"Capability {cudaOutput}"; IsOk = true })
        else
            results.Add({ Name = "NVIDIA GPU"; Status = "Not Found"; Details = "nvidia-smi not available or no GPU"; IsOk = true })
    with _ ->
        results.Add({ Name = "NVIDIA GPU"; Status = "Not Found"; Details = "nvidia-smi not installed"; IsOk = true })
    ...
    try
        ...
        if not (String.IsNullOrEmpty(rocmPath)) && Directory.Exists(rocmPath) then
            results.Add({ Name = "AMD ROCm"; Status = "Detected"; Details = rocmPath; IsOk = true })
    with _ -> ()
    ...
    if not (String.IsNullOrEmpty(cudaHome)) then
        results.Add({ Name = "CUDA_HOME"; Status = "Set"; Details = cudaHome; IsOk = true })
    elif not (String.IsNullOrEmpty(cudaPath)) then
        results.Add({ Name = "CUDA_PATH"; Status = "Set"; Details = cudaPath; IsOk = true })

    if results.Count = 0 then
        results.Add({ Name = "GPU Acceleration"; Status = "Not Available"; Details = "No GPU detected - using CPU"; IsOk = true })
...
let private getDiskInfo () =
    try
        ...
        let usedPercent = 100.0 - (float drive.AvailableFreeSpace / float drive.TotalSize * 100.0)

        let status = if freeGB > 10L then "OK" else if freeGB > 5L then "Low" else "Critical"
        let isOk = freeGB > 5L
    ...
    with _ ->
        [ { Name = "Disk"; Status = "Unknown"; Details = "Could not read disk info"; IsOk = true } ]
...
let private checkNetworkConnectivity (verbose: bool) =
    task {
        ...
        let endpointsToCheck = endpoints |> List.filter (fun (_, _, shouldCheck) -> shouldCheck)
            ...
            |> List.map (fun (name, url, _) ->
                task {
                    try
                        ...
                        return { Name = name; Status = "Reachable"; Details = $"{url} ({sw.ElapsedMilliseconds}ms, HTTP {code})"; IsOk = true }
                    with
                    | :? TaskCanceledException ->
                        return { Name = name; Status = "Timeout"; Details = $"{url} - Timed out after 5s"; IsOk = false }
                    | :? HttpRequestException as ex ->
                        let msg = if ex.Message.Length > 50 then ex.Message.Substring(0, 50) + "..." else ex.Message
                        return { Name = name; Status = "Unreachable"; Details = $"{url} - {msg}"; IsOk = false }
                    | ex ->
                        let msg = if ex.Message.Length > 40 then ex.Message.Substring(0, 40) + "..." else ex.Message
                        return { Name = name; Status = "Error"; Details = $"{url} - {ex.GetType().Name}: {msg}"; IsOk = false }
                }
            )
            |> Task.WhenAll
...
let private getTarsInfo () =
    ...
    [
        { Name = "Workspace"; Status = "Info"; Details = workspaceDir; IsOk = true }
        { Name = "TARS v2 Directory"; Status = (if hasV2 then "Found" else "Missing"); Details = v2Dir; IsOk = hasV2 }
        { Name = "Config (tars.json)"; Status = (if hasTarsConfig then "Found" else "Not Found"); Details = "(optional)"; IsOk = true }
        { Name = "Secrets File"; Status = (if hasSecretsFile then "Found" else "Not Found"); Details = "(optional - use env vars instead)"; IsOk = true }
    ]
...
let runWithVerbose (logger: ILogger) (verbose: bool) =
    task {
        ...
        if verbose then Console.info "(Verbose mode - showing additional details)"
        ...
        for r in gpuInfo do
            match r.Status with
            | "Detected" | "Available" | "Set" -> Console.checkOk r.Name r.Details
            | "Not Found" | "Not Available" -> Console.checkWarn r.Name r.Details
        ...
        if not ollamaResult.IsOk then issues <- issues + 1
        ...
        if lowDisk then issues <- issues + 1
