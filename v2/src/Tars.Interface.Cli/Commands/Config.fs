...
let show () =
    ...
    for (cliKey, envKey, description) in configKeys do
        ...
        | Some (source, value) ->
            let displayValue = 
                if cliKey.Contains("password") then mask value
                else value
            table.AddRow(cliKey, $"[green]{Markup.Escape(displayValue)}[/]", $"[dim]{source}[/]") |> ignore
        | None ->
            table.AddRow(cliKey, "[red]not set[/]", "[dim]n/a[/]") |> ignore
        ...
        try
            ...
            if response.IsSuccessStatusCode then
                ...
                try
                    ...
                    | _ ->
                        AnsiConsole.MarkupLine($"[dim]Response: {content.Substring(0, Math.Min(200, content.Length))}...[/]")
                with _ ->
                    AnsiConsole.MarkupLine($"[dim]Response: {content.Substring(0, Math.Min(200, content.Length))}...[/]")
                ...
                if response.StatusCode = System.Net.HttpStatusCode.Unauthorized ||
                   response.StatusCode = System.Net.HttpStatusCode.Forbidden then
