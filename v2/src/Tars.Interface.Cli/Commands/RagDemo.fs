...
module Config =
    ...
    let [<Literal>] SemanticKeywordGroups = 10
    /// Multiplier for keyword dimension spread
    let [<Literal>] KeywordDimensionSpread = 7
    /// Preview length for document content display
    let [<Literal>] ContentPreviewLength = 100
    /// Progress bar width in characters
    let [<Literal>] ProgressBarWidth = 30
    /// Default collection name for demo documents
    let [<Literal>] CollectionName = "tars_docs"
    /// Maximum results to display per scenario
    let [<Literal>] MaxDisplayResults = 8
    /// Maximum attributions to display
    let [<Literal>] MaxDisplayAttributions = 3
    /// Default benchmark iterations
    let [<Literal>] DefaultBenchmarkRuns = 3
...
let private sampleDocuments = [
    ...
    ("doc-arch-001", "TARS Architecture Overview",
     "TARS (Transformative Autonomous Reasoning System) is a multi-agent AI framework built in F#. \
      ...
      self-improvement through genetic algorithms and A/B testing.", "architecture", DateTime.UtcNow.AddDays(-5.0))

    ("doc-arch-002", "Agent Communication Protocol",
     ...
     "architecture", DateTime.UtcNow.AddDays(-10.0))

    // RAG documents
    ("doc-rag-001", "RAG Pipeline Overview",
     "The Retrieval Augmented Generation (RAG) pipeline in TARS uses a sophisticated 18-step process. \
      ...
      or full LLM reranking for maximum precision.", "rag", DateTime.UtcNow.AddDays(-1.0))

    ("doc-rag-002", "Vector Store Implementation",
     "TARS includes an in-memory vector store with SIMD-optimized cosine similarity calculations. \
      ...
      Embeddings are cached using an LRU strategy to avoid redundant API calls.", "rag", DateTime.UtcNow)

    ("doc-rag-003", "Knowledge Graph Integration",
     "The Knowledge Graph in TARS stores relationships between concepts, agents, files, and tasks. \
      ...
      but is semantically connected through the knowledge graph structure.", "rag", DateTime.UtcNow.AddDays(-3.0))

    // Metascript documents
    ("doc-meta-001", "Metascript Workflow Language",
     ...
     "metascript", DateTime.UtcNow.AddDays(-7.0))

    ("doc-meta-002", "Metascript Step Types",
     "TARS Metascript supports four step types: 1) Agent steps invoke LLM with instructions and context, \
      ...
      hybrid search, query expansion, reranking, and metadata filtering.", "metascript", DateTime.UtcNow.AddDays(-2.0))

    // Evolution documents
    ("doc-evol-001", "Self-Improvement Engine",
     "The Evolution module enables TARS to improve itself through experimentation. \
      ...
      measures task completion, response quality, and efficiency metrics.", "evolution", DateTime.UtcNow.AddDays(-15.0))

    // Security documents  
    ("doc-sec-001", "Security and Credentials",
     "TARS uses a CredentialVault for secure secret management with encryption at rest. \
      ...
      All external API calls use HTTPS with certificate validation.", "security", DateTime.UtcNow.AddDays(-20.0))

    // Recent update
    ("doc-update-001", "Latest RAG Improvements",
     "Recent updates to TARS RAG include: contextual compression to extract relevant portions, \
      ...
      on query classification.", "rag", DateTime.UtcNow)
]
...
let private loadCustomDocuments (folderPath: string) =
    ...
    let files =
        ...
        |> List.collect (fun pattern ->
            Directory.GetFiles(folderPath, pattern, SearchOption.AllDirectories) |> Array.toList)
        |> List.distinct
    ...
    |> List.mapi (fun idx filePath ->
        ...
        let category =
            ...
            if String.IsNullOrEmpty(dir) then "custom" else dir.Replace(Path.DirectorySeparatorChar, '/')
...
module Console =
    ...
    let subHeader text = writeLine ConsoleColor.Yellow $"\n▶ {text}"

    /// Display informational text
    let info text = writeLine ConsoleColor.White $"  {text}"

    /// Display success message
    let success text = writeLine ConsoleColor.Green $"  ✓ {text}"

    /// Display warning message
    let warning text = writeLine ConsoleColor.Yellow $"  ⚠ {text}"

    /// Display highlighted text
    let highlight text = writeLine ConsoleColor.Magenta $"  ★ {text}"

    /// Display dimmed/secondary text
    let dim text = writeLine ConsoleColor.DarkGray $"    {text}"

    /// Display verbose output (only when verbose mode is enabled)
    let verbose (isVerbose: bool) text =
        if isVerbose then writeLine ConsoleColor.DarkCyan $"    [v] {text}"

    /// Display a bullet point
    let bullet text = writeLine ConsoleColor.White $"    • {text}"

    /// Display a result line
    let result text = writeLine ConsoleColor.Cyan $"  → {text}"

    /// Display an error message
    let error text = writeLine ConsoleColor.Red $"  ✗ {text}"

    /// Display a score with color based on value (green=high, yellow=medium, red=low)
    let coloredScore (score: float32) =
        let color =
            if score >= Config.HighScoreThreshold then ConsoleColor.Green
            elif score >= Config.MediumScoreThreshold then ConsoleColor.Yellow
    ...
    let progressBarWithEta (current: int) (total: int) (label: string) =
        if current = 1 then progressStartTime <- DateTime.UtcNow

        let pct = float current / float total
        let filled = int (pct * float Config.ProgressBarWidth)
        let bar = String.replicate filled "█" + String.replicate (Config.ProgressBarWidth - filled) "░"
        ...
        let eta =
            if current > 0 && current < total then
                ...
                if remaining < 60.0 then sprintf " ETA: %.0fs" remaining
                else sprintf " ETA: %.1fm" (remaining / 60.0)
            else ""

        Console.Write($"\r  [{bar}] {current}/{total} {label} ({pctStr}){eta}    ")
        if current = total then Console.WriteLine()
...
let private semanticKeywords = [|
    "agent"; "communication"; "message"; "protocol"; "async"
    "vector"; "embedding"; "similarity"; "search"; "retrieval"
    "rag"; "augmented"; "generation"; "context"; "knowledge"
    "metascript"; "workflow"; "step"; "pipeline"; "execution"
    "evolution"; "genetic"; "mutation"; "fitness"; "optimization"
    "security"; "sandbox"; "permission"; "isolation"; "trust"
    "graph"; "node"; "edge"; "traversal"; "hop"
    "simd"; "performance"; "cache"; "batch"; "parallel"
    "llm"; "model"; "prompt"; "completion"; "token"
    "architecture"; "module"; "component"; "system"; "design"
|]
...
let private createSemanticEmbedding (text: string) =
    ...
    let embedding = Array.zeroCreate<float32> Config.EmbeddingDimension
    let spread = Config.KeywordDimensionSpread
    ...
    if magnitude > 0.0f then
        ...
        let e = Array.init Config.EmbeddingDimension (fun _ -> float32 (rng2.NextDouble() * 2.0 - 1.0))
...
type DemoLlm() =
    ...
    interface ILlmService with
        member _.CompleteAsync(req) =
            task {
                let content = if req.Messages.Length > 0 then req.Messages.[0].Content else ""
...
type LiveLlm(chatModel: string, embedModel: string) =
    ...
    let baseUri =
        ...
        if String.IsNullOrEmpty(url) then Uri("http://localhost:11434")
        else Uri(url)
...
let private baseConfig = { RagConfig.Default with CollectionName = Config.CollectionName; MinScore = 0.0f }
...
module ConfigBuilders =
    ...
    let hybrid topK weight =
        { baseConfig with TopK = topK; EnableHybridSearch = true; SemanticWeight = weight }

    /// Query expansion config
    let withExpansion topK count =
        { baseConfig with TopK = topK; EnableQueryExpansion = true; QueryExpansionCount = count }

    /// Query routing config
    let withRouting topK =
        { baseConfig with TopK = topK; EnableQueryRouting = true }

    /// Time decay config
    let withTimeDecay topK halfLifeDays =
        { baseConfig with TopK = topK; EnableTimeDecay = true; TimeDecayHalfLifeDays = halfLifeDays }

    /// Metadata filter config
    let withFilter topK field op value =
        { baseConfig with TopK = topK; MetadataFilters = [{ Field = field; Operator = op; Value = value }] }

    /// RRF config
    let withRRF topK =
        { baseConfig with TopK = topK; EnableHybridSearch = true; EnableRRF = true; RRFConstant = 60 }
    ...
    let withCrossEncoder topK =
        { baseConfig with TopK = topK; EnableCrossEncoder = true; CrossEncoderModel = "cross-encoder-demo" }

    /// LLM reranking config
    let withLlmRerank topK =
        { baseConfig with TopK = topK; EnableReranking = true }

    /// Compression config
    let withCompression topK maxChars =
        { baseConfig with TopK = topK; EnableContextualCompression = true; CompressionMaxChars = maxChars }

    /// Sentence window config
    let withSentenceWindow topK windowSize =
        { baseConfig with TopK = topK; EnableSentenceWindow = true; SentenceWindowSize = windowSize }
...
let private runCompareMode (llm: ILlmService) (vectorStore: IVectorStore) (options: DemoOptions) (scenariosToRun: (int * DemoScenario) list) =
    task {
        ...
        let avgLatencyDiff = comparisons |> List.averageBy (fun c -> float c.LatencyDifference)
...
let private scenarios = [
    ...
    { Name = "Full Pipeline (All Features)"
      ...
      ExpectedFeatures = ["Hybrid search"; "Query expansion"; "Query routing"; "Time decay"; "RRF"; "Cross-encoder"; "Attribution"; "Metrics"] }
]
...
let private indexDocuments (llm: ILlmService) (vectorStore: IVectorStore) =
    task {
        ...
        let! _ =
            sampleDocuments
            |> List.indexed
            |> List.fold (fun (acc: Task<int>) (idx, (id, title, content, category, timestamp)) ->
                task {
                    let! count = acc
                    ...
                    do! vectorStore.SaveAsync(Config.CollectionName, id, embedding, metadata)
...
let private parseResults (context: string) (results: obj list) : RetrievalResult list =
    ...
    lines
    |> Array.indexed
    |> Array.fold (fun (acc, currentRank) (idx, line) ->
        if line.StartsWith("[") && line.Contains("score:") then
            ...
            let content =
                if idx + 1 < lines.Length then
                    ...
                    if nextLine.Length > Config.ContentPreviewLength then
                        nextLine.Substring(0, Config.ContentPreviewLength) + "..."
                    else nextLine
                else ""
    ...
    |> fst
    |> List.rev
    |> List.truncate Config.MaxDisplayResults
...
let private runScenario
    ...
    task {
        ...
        try
            ...
            if isTextMode then Console.subHeader "Executing Retrieval..."
            let sw = System.Diagnostics.Stopwatch.StartNew()
            let! state = Tars.Metascript.Engine.run ctx workflow Map.empty
            sw.Stop()

            if isTextMode then Console.subHeader "Results"
            ...
            if isTextMode then
                ...
                |> List.iter (fun r ->
                    ...
                    if r.Content.Length > 0 then Console.dim r.Content
                )
            ...
            if isTextMode && scenario.Config.EnableAnswerAttribution then
                ...
                | Some attrs ->
                    ...
                    attrList
                    |> List.truncate Config.MaxDisplayAttributions
                    |> List.iter (fun attr ->
            ...
            if isTextMode then
                scenario.Config.Metrics |> Option.iter (fun m ->
            ...
            if isTextMode then Console.success "Scenario complete"
        ...
        with ex ->
            if isTextMode then
                ...
                let stackPreview =
                    if isNull ex.StackTrace then "(no stack trace)"
                    elif ex.StackTrace.Length > 200 then ex.StackTrace.Substring(0, 200) + "..."
                    else ex.StackTrace
...
let private displayAggregateSummary (metrics: AggregateMetrics) =
    ...
    if metrics.TotalCacheHits + metrics.TotalCacheMisses > 0L then
        let hitRate = float metrics.TotalCacheHits / float (metrics.TotalCacheHits + metrics.TotalCacheMisses) * 100.0
...
let runWithOptions (logger: ILogger) (options: DemoOptions) =
    task {
        ...
        if isTextMode then
            ...
            if options.Quick then Console.info "(Running in quick mode - no prompts)"
            if options.Verbose then Console.info "(Verbose mode enabled)"
            if options.BenchmarkRuns > 0 then Console.info $"(Benchmark mode: {options.BenchmarkRuns} runs per scenario)"
        ...
        let llm =
            if options.UseLiveLlm then
                if isTextMode then Console.info "(Using LIVE Ollama LLM - responses may take longer)"
        ...
        let! _ =
            ...
            |> List.fold (fun (acc: Task<int>) (idx, (id, title, content, category, timestamp)) ->
                task {
                    ...
                    do! vectorStore.SaveAsync(Config.CollectionName, id, embedding, metadata)
                    if isTextMode then Console.progressBarWithEta (idx + 1) documents.Length "documents"
                    return idx + 1
                }
            ) (Task.FromResult 0)

        if isTextMode then Console.waitForEnter options.Quick "Press ENTER to start the demo scenarios..."
        ...
        let! outcomes =
            scenariosToRun
            |> List.fold (fun (acc: Task<ScenarioOutcome list>) (scenarioNum, scenario) ->
                task {
                    ...
                    if options.BenchmarkRuns > 0 then
                        let! latencies =
                            ...
                            |> List.fold (fun (latAcc: Task<int64 list>) runNum ->
                                task {
                                    let! prevLats = latAcc
                                    if isTextMode && runNum > 1 then
                                        Console.dim $"  Benchmark run {runNum}/{options.BenchmarkRuns}..."
                                    let! outcome = runScenario llm vectorStore scenario scenarioNum options aggregateMetrics
                                    let latency = match outcome with Success (_, ms) -> ms | _ -> 0L
                        ...
                        if isTextMode then
                            ...
                            Console.dim $"Runs: {stats.Runs}, Min: {stats.MinLatencyMs}ms, Max: {stats.MaxLatencyMs}ms"
        ...
        let successCount = outcomes |> List.filter (function Success _ -> true | _ -> false) |> List.length
