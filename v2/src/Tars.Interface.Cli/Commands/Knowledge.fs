...
let private getKnowledgePath () =
    // Look for knowledge folder relative to current directory or v2
    let candidates = [
    ...
    candidates |> List.tryFind Directory.Exists |> Option.defaultValue candidates.[0]

let private printEntry (entry: Entry) =
    let catStr = match entry.Category with
                 | Beliefs -> "beliefs" | Learned -> "learned" 
                 | Facts -> "facts" | Meta -> "meta" | Custom s -> s
    let confStr = match entry.Confidence with
...
let private printEntryFull (entry: Entry) =
    ...
    printfn "Created: %s | Updated: %s" (entry.CreatedAt.ToString("yyyy-MM-dd")) (entry.UpdatedAt.ToString("yyyy-MM-dd"))
    Console.ResetColor()
    printfn ""
...
let run (options: KnowledgeOptions) =
    ...
    | "search" | "find" ->
        ...
        | None ->
            printfn "Usage: tars knowledge search <query>"
    ...
    | "show" | "get" ->
        ...
        | None ->
            printfn "Usage: tars knowledge show <id>"
    
    | "add" | "new" ->
        match options.Title, options.Content with
        | Some title, Some content ->
            let category = options.Category |> Option.map parseCategory |> Option.defaultValue Learned
            let tags = options.Tags |> Option.map (fun t -> t.Split(',') |> Array.map (_.Trim()) |> Array.toList) |> Option.defaultValue []
            let entry = kb.Add(title, content, category, tags = tags, source = Told)
            Console.ForegroundColor <- ConsoleColor.Green
            printfn "✓ Created entry: %s" entry.Id
            Console.ResetColor()
        | _ ->
            printfn "Usage: tars knowledge add --title \"Title\" --content \"Content\" [--category beliefs|learned|facts|meta] [--tags \"tag1,tag2\"]"
    ...
    | "delete" | "rm" ->
        ...
        | None ->
            printfn "Usage: tars knowledge delete <id>"
    
    | "path" ->
        printfn "Knowledge base path: %s" kb.BasePath
