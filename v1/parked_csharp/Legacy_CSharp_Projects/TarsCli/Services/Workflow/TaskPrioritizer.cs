...
using CodeAnalysisIssueSeverity = TarsCli.Services.CodeAnalysis.IssueSeverity;
using CodeAnalysisIssueType = TarsCli.Services.CodeAnalysis.CodeIssueType;
...
public class TaskPrioritizer
{
    ...
    public List<CodeIssue> PrioritizeIssues(List<CodeIssue> issues, int maxIssues = 100)
    {
        ...
        try
        {
            ...
            {
                var analysisIssue = CodeIssueAdapter.ToCodeAnalysisCodeIssue(issue);
                var score = CalculateIssuePriorityScore(analysisIssue);
    ...
    private double CalculateFilePriorityScore(CodeAnalysisResult analysisResult)
    {
        ...
        foreach (var issue in analysisResult.Issues)
        {
            ...
            {
                case CodeAnalysisIssueSeverity.Critical:
                    score += 10;
                    break;
                case CodeAnalysisIssueSeverity.Error:
                    score += 5;
                    break;
                case CodeAnalysisIssueSeverity.Warning:
                    score += 2;
                    break;
                case CodeAnalysisIssueSeverity.Info:
                    score += 1;
                    break;
            }
        }
        ...
        var securityIssues = analysisResult.Issues.Count(i => i.Type == CodeAnalysisIssueType.Security);
        var performanceIssues = analysisResult.Issues.Count(i => i.Type == CodeAnalysisIssueType.Performance);
        var reliabilityIssues = analysisResult.Issues.Count(i => i.Type == CodeAnalysisIssueType.Reliability);
    ...
    /// <returns>Priority score</returns>
    private double CalculateIssuePriorityScore(CodeAnalysis.CodeIssue issue)
    {
