...
open TarsEngine.FSharp.SelfImprovement.OllamaImprovementApplier
open ModelSelection

// Import types from other modules
open TarsEngine.FSharp.SelfImprovement.CodeAnalysisStep
    ...
    let getHandler (logger: ILogger) (maxImprovements: int) : WorkflowState -> Task<StepResult> =
        fun state ->
            task {
                ...
                logger.LogInformation("Using improvement model {ModelName} from provider {Provider}", ollamaModel, selection.Provider)
                ...
                let mutable ollamaApplied = false

                for opportunity in limitedOpportunities do
                    try
                        let patternReplacement = getPatternReplacement knowledgeBase opportunity.PatternId
                        let canUseOllama =
                            useOllama
                            && patternReplacement.IsSome
                            && opportunity.CodeSnippet.IsSome

                        let! appliedImprovement =
                            if canUseOllama then
                                task {
                                    logger.LogInformation("Applying improvement using Ollama: {FilePath}", opportunity.FilePath)

                                    let ollamaOpportunity: OllamaImprovementOpportunity = {
                                        FilePath = opportunity.FilePath
                                        Type = opportunity.PatternName
                                        Description = $"Applying knowledge-base replacement for pattern {opportunity.PatternId}"
                                        CurrentCode = opportunity.CodeSnippet |> Option.defaultValue ""
                                        ImprovedCode = patternReplacement |> Option.defaultValue ""
                                        Rationale = $"Pattern {opportunity.PatternId} recommends replacing the identified snippet. Use the provided improved code while preserving surrounding context."
                                        Confidence = opportunity.Confidence
                                    }

                                    let! result = applyImprovementWithOllama logger ollamaOpportunity ollamaEndpoint ollamaModel
                                    ...
                                    | Some _ ->
                                        ...
                                        return result
                                    | None ->
                                        logger.LogInformation("Falling back to deterministic replacement for {FilePath}", opportunity.FilePath)
                                        return! applyImprovement logger knowledgeBase opportunity
                                }
                            else
                                if useOllama then
                                    logger.LogInformation("Ollama enabled but missing replacement data for {PatternId}; using deterministic path.", opportunity.PatternId)
                ...
                let resultMap = Map.ofList [
                    ...
                    "used_ollama", (if ollamaApplied then "true" else "false")
