...
open TarsEngine.FSharp.Cli.AI.InferencePipeline

/// TARS AI Inference Engine Core - Main neural network inference engine
module TarsAIInferenceEngineCore =
    
    /// TARS AI Inference Engine
    type TarsAIInferenceEngine(logger: ITarsLogger, cudaEngine: UnifiedCudaEngine) =
        ...
        member this.InitializeAsync(cancellationToken: CancellationToken) =
            task {
                try
                    ...
                    logger.LogInformation(correlationId, "🧠 Initializing TARS AI Inference Engine")
                ...
                with
                | ex ->
                    let error = ExecutionError ("AI inference engine initialization failed", Some ex)
            ...
            task {
                try
                    ...
                    | true, model ->
                        ...
                        let! inferenceResult = this.ExecuteInferencePipeline(model, request, cancellationToken)
        ...
        member this.GetCapabilities() : string list =
            [
                "🧠 Complete neural network inference with CUDA acceleration"
                "🔥 Custom TARS model format with optimized serialization"
                "⚡ Real-time GPU-accelerated tensor operations"
                "🎯 Transformer, CNN, RNN, and custom architecture support"
                "📊 Comprehensive performance monitoring and metrics"
                "🔄 Dynamic model loading and unloading"
                "🎛️ Configurable inference parameters and optimization"
                "🔗 Seamless integration with TARS ecosystem"
        ...
        /// Execute inference pipeline
        member private this.ExecuteInferencePipeline(model: TarsModel, request: InferenceRequest, cancellationToken: CancellationToken) =
            InferencePipeline.executeInferencePipeline logger model request cancellationToken
        ...
        interface IDisposable with
            member this.Dispose() =
                loadedModels.Clear()
                modelMetrics.Clear()
                activeInferences.Clear()

    /// Create TARS AI inference engine
    let createAIInferenceEngine (logger: ITarsLogger) (cudaEngine: UnifiedCudaEngine) =
        new TarsAIInferenceEngine(logger, cudaEngine)
