...
public class DslService
{
    ...
    /// <summary>
    /// Logs detailed information about a DSL block
    /// </summary>
    /// <param name="block">The block to log</param>
    private void LogBlockDetails(TarsEngine.DSL.Ast.TarsBlock block)
    {
        // Create a structured log message with block details
        var blockInfo = new StringBuilder();

        // Add block type
        blockInfo.AppendLine($"Block type: {block.Type}");
        ...
        try
        {
            ...
            {
                blockInfo.AppendLine($"  Name: {name}");
            }
        }
        catch
        {
            // Ignore errors with F# option handling
        }

        // Add block properties count
        try
        {
            var propertiesCount = block.Properties.Count;
            blockInfo.AppendLine($"  Properties: {propertiesCount}");

            // Add some key properties if available
            if (propertiesCount > 0)
            {
                blockInfo.AppendLine("  Key properties:");

                foreach (var key in block.Properties.Keys.Take(5))
                {
                    blockInfo.AppendLine($"    - {key}");
                }

                if (propertiesCount > 5)
                {
                    blockInfo.AppendLine($"    ... and {propertiesCount - 5} more");
                }
            }
        }
        catch
        {
            // Ignore errors with F# map handling
        }

        // Add nested blocks count
        try
        {
            var nestedBlocksCount = block.NestedBlocks.Length;
            if (nestedBlocksCount > 0)
            {
                blockInfo.AppendLine($"  Nested blocks: {nestedBlocksCount}");
            }
        }
        catch
        {
            // Ignore errors with F# list handling
        }

        // Log the block details
        _logger.LogInformation(blockInfo.ToString());
    }
