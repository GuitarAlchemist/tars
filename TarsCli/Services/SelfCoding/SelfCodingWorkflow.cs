...
public class SelfCodingWorkflow
{
    ...
    public async Task<bool> StartWorkflowAsync(List<string> targetDirectories)
    {
        ...
        try
        {
            ...
            {
                var workflowStatus = await _workflowCoordinator.GetWorkflowStatusAsync(_currentWorkflowId);
                if (workflowStatus.Status == WorkflowStatus.Running || workflowStatus.Status == WorkflowStatus.Paused)
                {
