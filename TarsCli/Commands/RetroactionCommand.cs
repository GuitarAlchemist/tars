...
namespace TarsCli.Commands
{
    ...
    public class RetroactionCommand
    {
        ...
        private readonly Services.MetascriptEngine _metascriptEngine;
        ...
        public RetroactionCommand(
            ...
            Services.MetascriptEngine metascriptEngine,
        ...
        public Command RegisterCommands()
        {
            ...
            var transformRules = new Option<string>(new[] { "--rules", "-r" }, "Path to the metascript rules file");
            var transformOutput = new Option<string>(new[] { "--output", "-o" }, "Output directory for transformed files");
            var transformRecursive = new Option<bool>(new[] { "--recursive", "-R" }, () => false, "Process directories recursively");
            var transformPreview = new Option<bool>(new[] { "--preview", "-p" }, () => false, "Preview changes without applying them");
            ...
            var compileOutput = new Option<string>(new[] { "--output", "-o" }, "Output assembly name");
            ...
            var generateOutput = new Option<string>(new[] { "--output", "-o" }, "Output F# file path");
            ...
            var agentTransformOutput = new Option<string>(new[] { "--output", "-o" }, "Output path for transformed file");
            ...
            var agentLearnAccepted = new Option<bool>(new[] { "--accepted", "-a" }, () => true, "Whether the transformation was accepted");
        ...
        private async Task TransformFileAsync(
            string filePath,
            System.Collections.Generic.List<Services.MetascriptEngine.TransformationRule> rules,
            string outputPath,
            bool preview)
        {
            try
        ...
        private void ShowDiff(string original, string modified)
        {
            ...
            var diff = new System.Collections.Generic.List<(int LineNumber, string OriginalLine, string ModifiedLine)>();
        ...
        private class NullReferenceAnalysisAgentAdapter : Services.ICodeAgent, Services.IAnalysisAgent
        {
            public string Name => "NullReferenceAnalyzer";
            public string Description => "Analyzes code for potential null reference exceptions";
            public Services.AgentRole Role => Services.AgentRole.Analysis;

            public async Task<Services.AnalysisResult> AnalyzeAsync(string filePath, string code)
            {
                // Create a simple analysis result
                var issues = new List<Services.CodeIssue>
                {
                    new Services.CodeIssue
                    {
                        Type = Services.IssueType.MissingExceptionHandling,
                        Location = "Line 10",
                        Description = "Potential null reference exception"
                    }
                };

                return new Services.AnalysisResult
                {
                    Issues = issues
                };
            }
        ...
        private class IneffectiveLoopAnalysisAgentAdapter : Services.ICodeAgent, Services.IAnalysisAgent
        {
            ...
            public async Task<Services.AnalysisResult> AnalyzeAsync(string filePath, string code)
            {
                ...
                var issues = new List<Services.CodeIssue>
                {
                    new Services.CodeIssue
                    {
                        Type = Services.IssueType.IneffectiveCode,
                        Location = "Line 20",
        ...
        private class NullCheckTransformationAgentAdapter : Services.ICodeAgent, Services.ITransformationAgent
        {
            public string Name => "NullCheckTransformer";
            public string Description => "Transforms code to add null checks";
            public Services.AgentRole Role => Services.AgentRole.Transformation;

            public async Task<Services.TransformationResult> TransformAsync(string filePath, string code, Services.CollaborativeAnalysisResult analysisResult)
            {
                ...
                return new Services.TransformationResult
                {
        ...
        private class LinqTransformationAgentAdapter : Services.ICodeAgent, Services.ITransformationAgent
        {
        ...
        private async Task<int> LearnFromTransformationAsync(string originalPath, string transformedPath, bool accepted)
        {
            try
            {
                ...
                {
                    if (agent is Services.ILearningAgent learningAgent)
                    {
