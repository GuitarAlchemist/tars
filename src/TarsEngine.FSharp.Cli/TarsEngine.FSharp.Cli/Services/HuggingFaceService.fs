        ...
        task {
            try
                ...
                | None ->
                    ...
                    | Error error ->
                        // Real error handling - no simulated responses
                        logger.LogWarning("Model not loaded, cannot generate response for {ModelId}", modelInfo.ModelId)

                        let response = $"ERROR: Model {modelInfo.Name} not loaded. Cannot generate response for prompt: '{prompt}'. Please ensure the model is properly loaded before making requests."
