...
public class ValueSystem
{
    ...
    private void InitializeValueWeights()
    {
        ...
        var coreWeight = 0.4 / 7; // 40% for core values (7 values)
        var growthWeight = 0.25 / 4; // 25% for growth values (4 values)
        var purposeWeight = 0.2 / 3; // 20% for purpose values (3 values)
        var wellbeingWeight = 0.15 / 3; // 15% for well-being values (3 values)
    ...
    public Task<ValueAlignment?> AlignValuesAsync()
    {
        if (!_isInitialized || !_isActive)
        {
            return Task.FromResult<ValueAlignment?>(null);
        }
        ...
        try
        {
            ...
            return Task.FromResult<ValueAlignment?>(alignment);
        }
        catch (Exception ex)
        {
    ...
    public ValueEvaluation EvaluateAction(string action, Dictionary<string, object>? context = null)
    {
        ...
        {
            var alignment = EvaluateActionAgainstValue(lowerAction, valueName);
    ...
    public bool UpdateValueWeight(string name, double weight)
    {
        ...
        // Normalize weights
        var totalWeight = _valueWeights.Values.Sum();
