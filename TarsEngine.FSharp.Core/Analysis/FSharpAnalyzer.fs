...
/// <summary>
/// Implementation of ILanguageAnalyzer for F# language.
/// </summary>
type FSharpAnalyzer(logger: ILogger<FSharpAnalyzer>, 
                    structureExtractor: FSharpStructureExtractor,
    ...
    member _.AnalyzeAsync(content: string, ?options: Map<string, string>) =
        task {
            try
                logger.LogInformation("Analyzing F# code")
                ...
                let metrics = Map.empty
                    ...
                    .Add("ModuleCount", structures |> List.filter (fun s -> s.StructureType = "module") |> List.length :> obj)
                    .Add("TypeCount", structures |> List.filter (fun s -> s.StructureType = "type") |> List.length :> obj)
                    .Add("RecordCount", structures |> List.filter (fun s -> s.StructureType = "record") |> List.length :> obj)
                    .Add("UnionCount", structures |> List.filter (fun s -> s.StructureType = "union") |> List.length :> obj)
                    .Add("FunctionCount", structures |> List.filter (fun s -> s.StructureType = "function") |> List.length :> obj)
                    .Add("ValueCount", structures |> List.filter (fun s -> s.StructureType = "value") |> List.length :> obj)
                    .Add("MemberCount", structures |> List.filter (fun s -> s.StructureType = "member") |> List.length :> obj)
                    ...
                    .Add("OpenCount", structures |> List.filter (fun s -> s.StructureType = "open") |> List.length :> obj)
            ...
            with
            | ex ->
                logger.LogError(ex, "Error analyzing F# code")
                return {
                    FilePath = None
                    Language = "fsharp"
                    Issues = [{
    ...
    member this.AnalyzeFileAsync(filePath: string, ?options: Map<string, string>) =
        task {
            try
                logger.LogInformation("Analyzing F# file: {FilePath}", filePath)
            ...
            with
            | ex ->
                logger.LogError(ex, "Error analyzing F# file: {FilePath}", filePath)
                return {
