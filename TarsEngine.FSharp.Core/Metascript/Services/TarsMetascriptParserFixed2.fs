...
module TarsMetascriptParser =
    ...
    /// </summary>
    let isBlockStart (line: string) : bool =
        let blockPatterns = [
    ...
    /// </summary>
    let isBlockEnd (line: string) : bool =
        line = "}"
    
    /// <summary>
    /// Parses parameter string into a map
    /// </summary>
    let parseParameters (paramStr: string) : Map<string, string> =
    ...
    /// </summary>
    let parseBlockHeader (line: string) : BlockType * Map<string, string> =
        // Extract block type
    ...
    /// </summary>
    let validateFSharpBlock (content: string) : Result<unit, string> =
        try
            ...
            for line in lines do
                ...
                if not (String.IsNullOrEmpty(trimmedLine)) && not (trimmedLine.StartsWith("//")) then
                    ...
                    if trimmedLine.Contains(";;") then
                        hasError <- true
                        errorMsg <- "F# interactive syntax (;;) not allowed in metascript blocks"
            
            if hasError then Error errorMsg else Ok ()
        with
        | ex -> Error ex.Message
    ...
    /// </summary>
    let validateTarsBlock (content: string) : Result<unit, string> =
        try
            let lines = content.Split([|'\n'; '\r'|], StringSplitOptions.RemoveEmptyEntries)
            ...
            for line in lines do
                ...
                if not (String.IsNullOrEmpty(trimmedLine)) && not (trimmedLine.StartsWith("//")) then
                    ...
                    if not isValidCommand then
                        hasError <- true
                        errorMsg <- sprintf "Unknown TARS command: %s" trimmedLine
