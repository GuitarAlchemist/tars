...
module YAMLBlockHandler =
    ...
    let updateStatusFile (filePath: string) (updates: Map<string, obj>) : Result<string, string> =
        try
            ...
            // Parse existing YAML
            let emptyContext : MetascriptExecutionContext = { Variables = Map.empty; ProjectPath = ""; OutputPath = "" }
            let existingData = 
                match parseYamlContent existingContent emptyContext with
                | Ok data -> data
                | Error _ -> Map.empty<string, obj>
            
            // Merge with updates - use explicit type annotations
            let mergedData = 
                let folder (acc: Map<string, obj>) (key: string) (value: obj) = acc.Add(key, value)
                updates |> Map.fold folder existingData
