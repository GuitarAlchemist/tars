                    ...
                    let nextConcept =
                        ...
                        if random.NextDouble() < 0.2 * associativeJumpingLevel then
                            ...
                            if List.isEmpty availableAssociations then
                                ...
                                let mutable cumulativeStrength = 0.0
                                let mutable chosen = (fst (List.last availableAssociations))
                                
                                for (concept, strength) in availableAssociations do
                                    cumulativeStrength <- cumulativeStrength + strength
                                    
                                    if cumulativeStrength >= randomValue then
                                        chosen <- concept
                                        break
