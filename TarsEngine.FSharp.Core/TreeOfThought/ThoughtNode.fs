...
/// Module containing types and functions for thought nodes
module ThoughtNode =
    ...
    let addChildren parent children =
        { parent with Children = children @ parent.Children }
    ...
    /// Compares two nodes by their scores
    let compareNodes node1 node2 =
        compare (getScore node1) (getScore node2)
    
    /// Gets the best child of a node based on score
    let getBestChild node =
        node.Children
        |> List.filter (fun child -> not child.Pruned)
        |> List.sortByDescending getScore
        |> List.tryHead
