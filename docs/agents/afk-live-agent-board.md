...
The AFK Live Agent Board is the repo-native, machine-readable source of truth for tracking parallel Away From Keyboard (AFK) work executed by cloud agents (like Jules or Codex).
...
5. **Deterministic Event Writes:** Keep event writes low-noise. Only record meaningful state transitions (e.g., CI failures, reviews), not internal agent monologues.
