...
type MetascriptService(
    ...
    // Execute F# code (simplified implementation)
    member private this.executeFSharpCode (code: string) (variables: Map<string, obj>) =
        task {
            try
                // For now, simulate F# execution with basic pattern matching
                // In a full implementation, this would use F# Interactive or compilation

                let output = System.Text.StringBuilder()
                let mutable newVariables = variables

                // Simple pattern matching for common F# constructs
                if code.Contains("printfn") then
                ...
                if code.Contains("let ") then
                    let letPattern = Regex(@"let\s+(\w+)\s*=\s*(.+)")
                    let matches = letPattern.Matches(code)
                    for m in matches do
